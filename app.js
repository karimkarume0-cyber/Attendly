...
function generatePDF() {
  ...
  const title = document.getElementById('title').value || 'Attendance Register';
  const date = document.getElementById('date').value || 'Date Not Set';
  const facilitator = document.getElementById('facilitator').value || 'N/A';
  ...
  doc.text(`Date: ${date}   |   Facilitator: ${facilitator}   |   Location: ${location}`, 14, 30);

  const tableRows = [];
  const signatures = []; 
  const rows = document.querySelectorAll('#participantBody tr');
  
  if (rows.length === 1 && rows[0].id === 'emptyRow') {
  ...
  rows.forEach((row) => {
    ...
    const org = cells[2].textContent.trim();
    ...
    const signatureData = imgEl ? imgEl.src : null;

    tableRows.push([no, name, org, designation, email, phone, ""]); 
    signatures.push(signatureData); 
  });

  doc.autoTable({
    startY: 40,
    head: [['No.', 'Participant', 'Organization', 'Designation', 'Email', 'Phone', 'Signature']],
    body: tableRows,
    theme: 'grid',
    headStyles: { fillColor: [39, 104, 232] }, 
    styles: { fontSize: 9, cellPadding: 4, valign: 'middle' },
    columnStyles: { 6: { cellWidth: 40, minCellHeight: 20 } },
    didDrawCell: function(data) {
      if (data.column.index === 6 && data.cell.section === 'body') {
        const base64Img = signatures[data.row.index];
        if (base64Img) {
          const dim = data.cell;
          const imgWidth = 35;
          const imgHeight = 15;
          const x = dim.x + (dim.width - imgWidth) / 2;
          const y = dim.y + (dim.height - imgHeight) / 2;
  ...
  doc.save(`${title.replace(/\s+/g, '_')}_Attendance.pdf`);
}
...
if (isStandalonePwa()) hideInstallButton();
