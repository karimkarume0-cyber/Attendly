...
function applyUrlMetadata() {
  ...
  if (isParticipantMode) {
    ...
    $("attendanceStats").hidden = true;
    $("export").hidden = true;
    $("history").hidden = true;
    $("historyNavItem").hidden = true;
    $("exportNavItem").hidden = true;
  ...
  if (!participants.length) {
    ...
    participants.forEach((person, index) => {
      ...
      row.innerHTML = `
        ...
        <td><button class="delete-link" title="Remove participant" aria-label="Remove ${escapeHtml(person.name)}" data-id="${person.submissionId}">×</button></td>`;
...
// ---------- Auth: participants sign in silently, admins log in ----------
...
function setAuthMode(mode) {
  ...
  $("authGateTitle").textContent = mode === "login" ? "Sign in to Attendly" : "Create your Attendly admin account";
...
function handleAdminAuthState(user) {
  ...
  $("appShellLayout").hidden = !loggedIn;
...
if (isFirebaseConfigured) {
  ...
  if (isParticipantMode) loadState();
  else { $("authGate").hidden = true; $("appShellLayout").hidden = false; }
}
