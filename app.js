...
function buildShareLink() {
  ...
  fields.forEach(key => {
    if (info[key]) url.searchParams.set(key, info[key]);
  });
  return url.toString();
}
...
if (isFirebaseConfigured) {
  onAuthStateChanged(auth, user => {
    currentUser = user;
    if (user) {
      setAuthPill("live", navigator.onLine ? "Central database sync" : "Offline (will sync)");
    }
  });
  signInAnonymously(auth).catch(error => {
    setAuthPill("error", "Sign-in failed — check Firebase setup");
    console.error(error);
  });
} else {
